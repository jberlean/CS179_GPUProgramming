...
__global__
void ja_forces_kernel(float * forces, float * vels_old, float * vels_new, float * data_old, float * data_new, float dt, int num_particles) {
  extern __shared__ float sdata[];
...
__global__
void ja_particles_kernel(float * forces, float * vels_old, float * vels_new, float * data_old, 
...
void call_interact_kernel(float dt) {
  ...
  else if (algorithm == JA)
  {
    float * forces;
    gpuErrChk(cudaMalloc((void **) &forces, num_particles * 2 * sizeof(float))); 
    gpuErrChk(cudaMemset(forces, 0, num_particles * 2 * sizeof(float)));

    ja_forces_kernel<<<num_blocks, num_threads_per_block, num_threads_per_block * sizeof(float) * 3>>>
                                                         (particle_vels[pingpong], particle_vels[1 - pingpong], 
                                                           particle_data[pingpong], particle_data[1 - pingpong], 
                                                           dt, num_particles);
    
    ja_particles_kernel<<<num_blocks, num_threads_per_block>>>(forces, particle_vels[pingpong], particle_vels[1 - pingpong], 
                                                           particle_data[pingpong], particle_data[1 - pingpong], 
                                                           dt, num_particles);
