...
void output_data(std::ofstream &out, float *particle_data, float *particle_vels, int frame_num, int num_particles, float width, float height) {
  ...
  for (int i = 0; i < num_particles; i++) {
    out << particle_data[i] << " " << particle_data[i + num_particles] << " " << particle_data[i + 2 * num_particles] << std::endl;
  }
}
...
void load_input_file(char *infile,
  ...
  for (int i = 0; i < num_particles; i++) {
    in >> particle_data[i] >> particle_data[i + num_particles] >> particle_data[i + 2 * num_particles]
        >> particle_vels[i] >> particle_vels[i + num_particles];
  }
